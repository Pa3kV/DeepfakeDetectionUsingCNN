...
falsePositivesPath = os.path.join(os.getcwd(), "results", "False Results", "False Positives")
falseNegativesPath = os.path.join(os.getcwd(), "results", "False Results", "False Negatives")

if not os.path.exists(falsePositivesPath):
    os.makedirs(falsePositivesPath)

if not os.path.exists(falseNegativesPath):
    os.makedirs(falseNegativesPath)

for id in falsePositives:
    imgPath = os.path.join(os.getcwd(), "frames", "test", testGenerator.filepaths[id])
    savePath = os.path.join(falsePositivesPath, filenames[id].split('\\')[1])
    shutil.copy(imgPath, savePath)

for id in falseNegatives:
    imgPath = os.path.join(os.getcwd(), "frames", "test", testGenerator.filepaths[id])
    savePath = os.path.join(falseNegativesPath, filenames[id].split('\\')[1])
    shutil.copy(imgPath, savePath)